...
HTTP GET / ─► satori(JSX, fonts)
           ...
           ─► 1-bit BMP (64 862 B)
           ─► Response(image/bmp, x-sleep-seconds: 120)
```
...
| Worker startup time | 36–46 ms | Reported by `wrangler deploy`. |
| Warm TTFB | median ~98 ms, p90 ~210 ms (n=20) | Single Wellington PoP, after one warm-up request. |
| Server-side wallTime (warm) | 53–87 ms | From `wrangler tail` `wallTime` field. |
...
| `src/index.ts` | Request handler — composes JSX → SVG → RGBA → BMP, returns response with `x-sleep-seconds: 120` |
...
1. **Implement client-side retry on 5xx.** One retry, 250 ms backoff. Defends against the cold-isolate hung-worker issue documented above.
2. **Confirm row order** by displaying `out.bmp` (or a synthetic top/bottom marker BMP) and verifying which edge appears at the top of the panel. If inverted, fix `to-bmp/index.ts` and `worker/src/bmp.ts` together.
3. **Measure end-to-end latency** wake → fetch → render → sleep. The 120 s `x-sleep-seconds` budget assumes everything fits in well under 1 s of awake-radio time.
4. **Implement `x-sleep-seconds` parsing and `esp_sleep_enable_timer_wakeup()`** call. Honour the header exactly; don't second-guess it client-side.
