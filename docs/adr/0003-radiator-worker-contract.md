...
Two pieces of metadata travel on every request: the **shared token** (auth) and the **radiator slug** (identity). They both live in request headers — `Authorization: Bearer <token>` and `X-Radiator-Slug` — rather than in the URL or a request body. Headers keep identity and credentials out of the addressing surface, and they let the radiator add future telemetry headers in the same namespace without changing the URL.

**Why the token rides in `Authorization`, not a custom `X-Radiator-Token`.** Cloudflare's observability pipeline captures every request header into Workers Logs, but it auto-redacts the well-known sensitive headers (`Authorization`, `Cookie`) to `********`. The original contract sent the secret as `X-Radiator-Token`, which CF logged in **cleartext** — anyone with Workers Observability → Read could read the shared token (GH #121). Moving the credential to `Authorization` keeps it out of logs while leaving the `X-Radiator-*` telemetry headers (slug, battery, hardware id) in cleartext, which is what the battery/uptime telemetry (#80) reads. We kept the static shared secret rather than replacing it with a signed/HMAC request: redaction is empirically confirmed (a probe against the live Worker stored the `Authorization` value as `********`), needs no firmware crypto or radiator clock, and the indistinguishable-401 and single-rotation properties below are unchanged. During rollout the Worker accepts either header; once every radiator sends `Authorization`, drop the `X-Radiator-Token` fallback in `api/auth.ts` and rotate `RADIATOR_SHARED_TOKEN` (the old token sat in logs).
