...
| `problem.{h,cpp}`   | Error-screen module ([ADR-0011](../../docs/adr/0011-error-contract-problem-details.md) / [ADR-0014](../../docs/adr/0014-error-screen-diagnostics-footer.md)): problem+json parse, fallback resolution, on-panel render + the diagnostics footer. Neutral `renderErrorScreen(…, ErrorDiag)` reusable by the Wi-Fi-down (#66) and transport-failure (#129) arms. |
| `version.h`         | `FIRMWARE_VERSION` — build-level version string (GH #61), shown on the error-screen footer and the wake banner. Bumped by hand; tracked (not per-deployment, so not in `settings.h`). |
...
**Diagnostics footer ([ADR-0014](../../docs/adr/0014-error-screen-diagnostics-footer.md) / GH #61).** Beneath the body the screen carries a footer answering *when / which*: the radiator **slug** + **firmware version** (`version.h`), the **SSID**, the **error time** (the `X-Server-Time` response header, sliced to `2026-05-23 06:48 UTC` — no on-device timezone), and the **next check** (resolved through the same `chooseSleep()` the sleep policy uses, so it can't drift from the real sleep). A warning glyph and `─` separator rules — both drawn from the already-bundled FiraSans — frame it. The fields ride in an `ErrorDiag` passed *into* the renderer, never reached out for. _Typography (a bold heading, a smaller body to fit more) is deferred — the library bundles one font face, so true bold/smaller needs generated headers; see ADR-0014._

Set `RADIATOR_VERBOSE 1` in `settings.h` to also render the raw `upstream_detail` snippet (carried on `metlink-*` errors) beneath the body — a debugging aid, off by default. The renderer takes neutral strings + a locally-filled `ErrorDiag`, not an HTTP object, so the worker-unreachable arms reuse it: a **Wi-Fi**-association failure ([#66](https://github.com/philipf/gotta-go/issues/66)) draws "No Wi-Fi connection", and a **transport** failure (DNS/TCP/TLS dead, dropped, timed out — no response, [#129](https://github.com/philipf/gotta-go/issues/129)) draws "No server connection" naming the cause. Both stay `HttpError` for the sleep/retry policy and carry no `X-Server-Time`, so their footer omits the error-time clause but still shows the next-check.
