...
export function toStopState(
  ...
  return {
    ...
    arrivals: raw.departures
      ...
      .filter((d) => destNames === undefined || destNames.some((n) => (d.destination?.name ?? '').toLowerCase().includes(n)))
      .map(toArrival),
  };
}
...
export function parseDelaySeconds(iso: string): number {
  ...
  const [, sign, h, mn, s] = m;
  const total = parseInt(h ?? '0', 10) * 3600 + parseInt(mn ?? '0', 10) * 60 + Math.round(parseFloat(s ?? '0'));
