...
describe('fetchArrivals', () => {
  it('surfaces a closed:true envelope as { kind: "closed" }', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(closedStop), { status: 200 });
  ...
  it('maps a scheduled-only departure into an Arrival with predicted === scheduled', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(scheduledTrain), { status: 200 });
  ...
  it('maps a delayed departure with delaySeconds, status, and predicted from arrival.expected', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(delayedTrain), { status: 200 });
  ...
  it('filters multi-route stop responses to the requested service_id', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(multiRouteBus), { status: 200 });
  ...
  it('array serviceId returns departures for all matched routes and excludes others', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(schoolBusMultiRoute), { status: 200 });
  ...
  it('omitted destinationStopId returns every departure for the matched service', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(churtonParkBranchingBus), { status: 200 });
    ...
    expect(result.data.arrivals.map((a) => a.tripHeadsign)).toEqual(['Churton Park', 'Grenada Village', 'Johnsonville West']);
  });
  ...
  it('array destinationStopId keeps departures bound for any listed terminus', async () => {
    ...
    expect(result.data.kind).toBe('open');
    if (result.data.kind !== 'open') return;
    expect(result.data.arrivals.map((a) => a.tripHeadsign)).toEqual(['Churton Park', 'Johnsonville West']);
  });

  it('omitted destinationNameIncludes returns every departure, expresses included', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(kapitiExpressMix), { status: 200 });
  ...
  it('surfaces a malformed JSON body on a 2xx as upstream, preserving the parse cause', async () => {
    const stubFetch: typeof fetch = async () => new Response('not even close to JSON', { status: 200 });
  ...
  it('surfaces other non-2xx as upstream with a body snippet in detail, without logging (#55)', async () => {
    ...
    const stubFetch: typeof fetch = async () => new Response('upstream exploded', { status: 500 });
  ...
  it('truncates an oversized upstream body to the 2 KB snippet cap in detail', async () => {
    const stubFetch: typeof fetch = async () => new Response('x'.repeat(3000), { status: 502 });
  ...
  it('surfaces a 4xx (other than 401/403/429) as client_error, distinct from upstream', async () => {
    const stubFetch: typeof fetch = async () => new Response('{"message":"Stop not found"}', { status: 404 });
  ...
  it('builds the request URL + x-api-key header and omits service_id from the query', async () => {
    ...
    expect(capturedUrl).toBe('https://api.opendata.metlink.org.nz/v1/stop-predictions?stop_id=TAKA1&limit=5');
  ...
  it.each([401, 403])('surfaces HTTP %i as auth with status + body detail', async (status) => {
    const stubFetch: typeof fetch = async () => new Response('{"message":"Forbidden"}', { status });
  ...
  it('falls back to departure.aimed when arrival.aimed is absent at origin stops', async () => {
    const stubFetch: typeof fetch = async () => new Response(JSON.stringify(originStop), { status: 200 });
