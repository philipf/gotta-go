...
export function failureResponse(err: unknown, init: FrameRequestContext & { obs: Record<string, unknown> }): Response {
  ...
  if (!(err instanceof AppError)) {
    fields.error = err instanceof Error ? { name: err.name, message: err.message, stack: err.stack } : { message: String(err) };
  }
