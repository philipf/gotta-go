// Error policy for priority_split_v2: maps Metlink gateway outcomes to
// AppErrors — config faults back off hard, transient blips retry at the phase
// sleep duration. Self-contained copy of v1's policy (issue #102): v2 fetches
// the same gateway and owns its own mapping so deleting v1 stays a clean folder
// delete. When v1 is removed this is the sole copy.
...
// Fatal, error.
export function metlinkBadRequest(upstreamStatus: number, stopId: string, upstreamDetail?: string): FatalError {
  return new FatalError({
...
export function toAppError(error: MetlinkGatewayError, target: TransitTarget): AppError {
  switch (error.kind) {
    ...
    case 'network':
      return metlinkUnavailable('Metlink is unreachable (network error). The radiator will retry on its next wake cycle.');
  }
}
