...
import type { ColumnViewModel, NoServiceColumn, PrioritySplitViewModel } from './viewmodel';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: s.hero, lineHeight: 1, marginTop: s.heroGap }}>{col.leaveIn}</div>
        <div style={{ fontSize: s.leaveBy, marginTop: s.heroGap }}>{col.leaveBy}</div>
      </div>
...
function noServiceColumn(col: NoServiceColumn, key: number, s: Sizing): ReactNode {
  return (
    ...
    >
      ...
      {col.nextDeparture ? <div style={{ fontSize: s.leaveBy, marginTop: s.heroGap * 2 }}>{col.nextDeparture}</div> : null}
    </div>
  );
}
