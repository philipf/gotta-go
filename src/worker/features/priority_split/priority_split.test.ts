...
import { preparePrioritySplitFrame, type ArrivalsSource, type PreparePrioritySplitFrameRequest } from './prepare-priority-split-frame';
...
function serviceColumn(target: TransitTarget, state: StopState, tz: string, now: Date): ServiceColumn {
...
describe('priority_split.column - Next services (the three after the hero)', () => {
  ...
  it('renders only the services that exist (no dash padding) - two after the hero', () => {
    const col = serviceColumn(
      busTarget,
      open(arrival('2026-05-22T19:42:00Z'), arrival('2026-05-22T19:54:00Z', '635'), arrival('2026-05-22T20:06:00Z')),
      TZ,
      NOW,
    );
  ...
  it('renders the single service after the hero as "NEXT hh:mm" (no separator)', () => {
    const col = serviceColumn(busTarget, open(arrival('2026-05-22T19:42:00Z'), arrival('2026-05-22T19:54:00Z', '635')), TZ, NOW);
...
describe('priority_split.column - Catchable selection', () => {
  it('skips a missed service (leave_by already passed) and selects the earliest catchable one', () => {
    ...
    const col = serviceColumn(busTarget, open(arrival('2026-05-22T19:32:00Z', '634'), arrival('2026-05-22T19:48:00Z', '635')), TZ, NOW);
...
describe('priority_split.preparePrioritySplitFrame - gateway failure -> throws problem type (#59)', () => {
  ...
  it('throws a Fatal metlink-bad-request on a client_error, naming the stop', async () => {
    const err = await prepareError(failingSource({ kind: 'client_error', status: 404, detail: '{"message":"Stop not found"}' }));
